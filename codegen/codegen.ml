...
(* Deterministic fresh names for de Bruijn binders: v0, v1, ... in traversal order.
   Determinism is required so the generated source is byte-stable (property P4/NF5). *)
let fresh =
  let c = ref 0 in
  fun () ->
    let n = !c in
...
(* Emit a single top-level [prog0 : unit -> unit] to stdout (captured by the dune rule). *)
let () =
  let body = emit_tm [] prog0 in
  Printf.printf "%slet prog0 () = %s\n" header body
