...
Twenty operations over one explicit `world`, grouped into ten families. Every family has a
...
`theories/EffIR.v` also declares the concurrency operations `OSpawn`, `OYield`, `OChanMake`,
`OChanSend`, and `OChanRecv`. They are Rocq-side only: no realizer, no code generation path,
no differential test. See the status section of the README.
