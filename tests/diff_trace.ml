...
let ref_obs (pairs : (Z.t * Z.t) list)
    : (Z.t * Rkv.Rval.t) list * Rkv.Rval.t list =
  let m0 =
    List.fold_left
      (fun m (k, v) ->
...
let fast_obs (pairs : (Z.t * Z.t) list)
  ...
  Rkv.Trace.run buf (fun () ->
      Rkv.Kv.run table (fun () -> ignore (Gen.sample_trace ())));
  (Rkv.Kv.observe table, Rkv.Trace.contents buf)

let list_eq eq a b = List.length a = List.length b && List.for_all2 eq a b
let state_eq = list_eq (fun (k1, v1) (k2, v2) -> Z.equal k1 k2 && Rkv.Rval.equal v1 v2)
