...
let write ~tag ~rocq_src ~theorem ~gen_code ~rkv ~rtr ~fkv ~ftr ~agree ~hex ~decoded ~roundtrip_ok =
 ...
 *{box-sizing:border-box} body{margin:0;background:var(--bg);color:var(--ink);font:15px/1.6 ui-sans-serif,system-ui,Segoe UI,Roboto,Arial}
 ...
 pre{background:#0c1120;border:1px solid var(--line);border-radius:8px;padding:12px 14px;overflow:auto;font:12.5px/1.5 ui-monospace,Menlo,Consolas,monospace;color:#cfe0ff}
...
|html}
      agree_cls (esc agree_txt) rt_cls (esc rt_txt) (esc rocq_src) (esc theorem) (esc gen_code) tag (show_kv fkv)
