...
- ⏭️ **Phase 3 — Planning (NEXT).** Produce `kb/plan.md`: 3–4 steps, vertical slice through the riskiest path first, plan-simulation gate, then user approval. **No implementation code until the plan is approved.**
...
## Exact next step
Begin Phase 3: draft `kb/plan.md` for the **KV vertical slice**, with Step 1 = the riskiest sub-task
(same EffIR value driving both the extracted reference interpreter and the codegen — prove a 3-node program
survives Rocq→extract→codegen→run before fleshing out `incr`). Run the plan-simulation gate (fresh subagent
walks the plan, lists ambiguities), then present to the user for approval.
