...
**Full design: [[adr-0019-concurrency]] (RESOLVED 2026-07-22).** C5.1 spike + full-tm adequacy CLOSED (theories/Cek.v — the CEK step machine equals big-step run over the WHOLE tm, axiom-free); C5 committed to the CEK machine. Scheduler built on the machine (theories/Sched.v): 5 conc ops (sequentially Dstuck, scheduler-intercepted), schedule oracle, channels, deadlock-as-Stuck; anti-vacuity green (schedule_matters/seq_embedding/deadlock/producer_consumer/spawn). General sequential embedding DONE (Cek.cek_drive_run + Sched.seq_embedding_general). **conc-free invariant + concurrent HTTP driver DONE 2026-07-23:** Sched §5c (conc_free/step_conc_free/run_to_sched_drive → conc_free_embeds + seq_embedding_cf: Hrun discharged by law for conc-free programs); theories/SchedHttp.v (http_driver_seq — certified http_prog recovers expected_log as a scheduled fiber, general; drv_concurrent_matches — a genuinely concurrent acceptor+worker over channels equals the sequential transcript under a run-to-completion schedule; drv_worker_starved mutant). All axiom-free. **OCaml runtime scheduler DONE 2026-07-27:** runtime/sched.ml (Effect.Deep cooperative scheduler, injected-schedule oracle, one-shot continuations, channels-only, deadlock=Stuck) + tests/diff_sched.ml (native realizer == extracted Sched.run_sched over 20 proven+adversarial schedules) + manifest [effect."Concurrency"]/Runtime_Sched_faithful + TCB-report rows. **Native concurrent HTTP server DONE 2026-07-27:** codegen conc ops (emit.ml -> Rkv.Sched.*); Samples.drv_acceptor/drv_worker generated (the proven handler); tools/rhttpd_conc.ml (Sched nested in the socket/store realizers over real TCP); tests/diff_sched_http.ml (concurrent server transcript == observe_sock reference over real loopback TCP, 4 cases). C5 complete end-to-end and deployable. Only reserved item: general (non-instance) re-derivation of drv_concurrent_matches through the channel plumbing (theory, no new TCB). The constraints below
