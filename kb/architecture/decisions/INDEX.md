...
| `adr-0019-concurrency.md` | **PROPOSED** (C5 capstone, awaiting user review): cooperative concurrency via a SCHEDULE ORACLE (the C4 script pattern generalized to interleaving); a suspended fiber = a defunctionalized continuation (a step machine over the SAME tm, load-bearing obligation = machine-vs-run adequacy); channels the only sharing (no data races representable); deterministic per schedule; OCaml 5 Effect.Deep backend; the C4 server made concurrent | C5 design (2026-07-22) |
