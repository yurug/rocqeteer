...
| `adr-0018-sockets.md` | Sockets (C4, SHIPPED 23af451): an injected CONNECTION SCRIPT oracle; ONE-SHOT half-close-driven connections (the file full-read loop deadlocks otherwise); 4 kernel ops + rider prim PFindSub; TCP contract = the named seam; a proven HTTP/1.0 server | C4 design (2026-07-22; implemented same day) |
| `adr-0019-concurrency.md` | **RESOLVED + DELIVERED** (C5 capstone): cooperative concurrency via a SCHEDULE ORACLE (the C4 script pattern generalized to interleaving); a suspended fiber = a defunctionalized continuation (a step machine over the SAME tm; machine-vs-run adequacy PROVEN); channels the only sharing (no data races representable); deterministic per schedule; OCaml 5 Effect.Deep backend; the C4 server made concurrent (native binary + real-TCP differential) | C5 design (2026-07-22), delivered end-to-end (2026-07-27) |
