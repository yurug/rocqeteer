...
echo "| prim_find_sub | primitive | Prims.prim_find_sub (first-occurrence substring scan; empty needle -> 0; the C4 rider prim, adr-0018) | diff_prims |"
echo "| prim_count_byte | primitive | Prims.prim_count_byte (count a byte, target mod 256; the C3 rider prim = wc -l with newline, adr-0017) | diff_prims |"
...
echo "| Journal (Append; buffer + optional sink, shared time source) | effect — DISCHARGED: derived(ElabNs.elab_ns_simulates), mode-F option (adr-0016 C2) | Rkv.Journal | diff_journal |"
...
echo "| Store (Get/Put/Delete/GetDeadline/SetDeadline; bytes keys, live iff now<=d) | effect — expiry surface DISCHARGED: derived(Elab.elab_simulates), mode-F performance option (adr-0016) | Rkv.Kv | diff_test, diff_kv, diff_store |"
echo "| Store KERNEL (Get/Put/Delete only; no deadline logic, no clock; deadline ops unhandled=loud) | effect — kernel-v1 (irreducible at this level, adr-0016 §6) | Rkv.Kv.run_kernel | diff_store_k |"
echo "| Expiry tower (ADR-0016 C1) | PROVEN discharge theorem (Elab.elab_simulates, axiom-free; < mutant rejected at the boundary) | theories/Elab.v | diff_store_k (3000 adversarial states; d-1/d/d+1 per key) |"
echo "| Consolidation tower (ADR-0016 C2) | PROVEN discharge theorem (ElabNs.elab_ns_simulates + elab_full_simulates, axiom-free; null-cache and forgetful-journal mutants rejected) | theories/ElabNs.v — cache/journal into the escaped kernel store (u/c/j regions); mode K = the COMPOSITION in generated/progk_generated.ml | diff_cache_k (3000: cold==warm==reference) + diff_journal_k (300x3x2: order, throw-prefix, timestamps) + diff_file/diff_sock mode-K legs (the tower COMPOSES with the file/socket families: the wc counter and the http buffer run through the u-region while real file/TCP ops pass through) |"
echo "| File (Open/Read/FWrite/Close; adr-0017 — inode pinning structural, EOF = empty chunk, modeled errors are values) | effect — kernel-v1 (syscall-backed) | Rkv.Fileio | diff_file |"
echo "| Runtime_FS_distinct_inodes | assumption (tcb-assumption, RUNTIME-CHECKED) | distinct tokens = distinct inodes; aliasing refused via (st_dev, st_ino) at open (cp posture) | diff_file |"
echo "| Runtime_FS_open_inode_stable | assumption (tcb-assumption) | held-open contents not externally written; path churn tolerated (fd pins inode); size/mtime change DETECTED at close (rsync posture) | diff_file |"
...
echo "| Error (Throw) | effect — kernel-v1 | Rkv.Err | diff_err |"
echo "| Runtime_Error_refines | assumption (tcb-assumption) | abort outcome+state | diff_err (3000) |"
echo "| Env (Ask) | effect — kernel-v1 | Rkv.Env | diff_env |"
echo "| Runtime_Env_refines | assumption (tcb-assumption) | read-only ctx flows | diff_env (3000) |"
echo "| Trace (Emit) | effect — kernel-v1 | Rkv.Trace | diff_trace |"
echo "| Runtime_Trace_refines | assumption (tcb-assumption) | event log in order | diff_trace (3000) |"
echo "| Cache (CGet/CPut) | effect — DISCHARGED: derived(ElabNs.elab_ns_simulates), mode-F performance option (adr-0016 C2) | Rkv.Cache | diff_cache |"
