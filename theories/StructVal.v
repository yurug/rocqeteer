...
Theorem tag_build_success :
  let '(o, _) := run_top (DInt 5) 0 sample_tag_build in
...
Theorem tag_build_overflow :
  let '(o, _) := run_top (DInt int64_max) 0 sample_tag_build in
...
Lemma tag_build_inhabited :
  exists w, run_top (DInt 5) 0 sample_tag_build =
            (ORet (DPair (DTag 0 (DInt 6)) (DList [DInt 42; DBytes tag_list_bytes])), w).
Proof.
  exists (snd (run_top (DInt 5) 0 sample_tag_build)).
...
Theorem tag_dispatch_zero :
  let '(o, _) := run_top (DTag 0 (DInt 1)) 0 sample_tag_dispatch in
  o = ORet (DBytes tag_dispatch_a_bytes).
Proof. vm_compute. reflexivity. Qed.

(** PTag 1 fires on a DTag 1 scrutinee. *)
Theorem tag_dispatch_one :
  let '(o, _) := run_top (DTag 1 (DBytes tag_list_bytes)) 0 sample_tag_dispatch in
...
Theorem tag_dispatch_default :
  let '(o, _) := run_top (DInt 42) 0 sample_tag_dispatch in
...
Theorem tag_collision_same_payload_diff_tag :
  (let '(o, _) := run_top (DTag 0 (DInt 1)) 0 sample_tag_dispatch in o)
  <>
  (let '(o, _) := run_top (DTag 1 (DInt 1)) 0 sample_tag_dispatch in o).
...
Theorem tag_dispatch_swapped_on_tag0 :
  let '(o, _) := run_top (DTag 0 (DInt 1)) 0 sample_tag_dispatch_swapped in
  o = ORet (DBytes tag_dispatch_b_bytes).
Proof. vm_compute. reflexivity. Qed.
...
Theorem tag_dispatch_swapped_differs_from_correct :
  (let '(o, _) := run_top (DTag 0 (DInt 1)) 0 sample_tag_dispatch in o)
  <>
  (let '(o, _) := run_top (DTag 0 (DInt 1)) 0 sample_tag_dispatch_swapped in o).
Proof. vm_compute. intro H. discriminate H. Qed.
...
Theorem tag_dispatch_wrong_tag_falls_default :
  let '(o, _) := run_top (DTag 7 (DInt 1)) 0 sample_tag_dispatch in
  o = ORet (DBytes tag_dispatch_default_bytes).
Proof. vm_compute. reflexivity. Qed.
...
Theorem tag_dispatch_notag_falls_default :
  let '(o, _) := run_top DUnit 0 sample_tag_dispatch in
  o = ORet (DBytes tag_dispatch_default_bytes).
Proof. vm_compute. reflexivity. Qed.
